    ...
    <div class="col-12">
        ...
        <div class="card card-table-border-none" id="recent-orders">
          <div class="card-header justify-content-between">
            <h2 class="pb-4">All Packages</h2>
            {{-- <div class="date-range-report ">
              <span>Jan 25, 2021 - Feb 23, 2021</span>
            </div> --}}
          </div>
          {{-- Filter --}}
          {{-- <form action="{{ route('package.search') }}" method="post" enctype="multipart/form-data" id="search">
            @csrf
            ...
            <table class="table card-table table-responsive table-responsive-large" style="width:100%" id="sampleTable">
              ...
              <tbody>
                @foreach ($packages as $package)
                    <tr>
                        <td>
                          <a class="text-dark" href="{{ route('package.edit', $package->id) }}">{{ $package->name }}</a>
                        </td>
                        {{-- <td class="d-none d-md-table-cell text-dark">{{ $package->name }}</td> --}}
                        <td class="d-none d-md-table-cell text-dark">{{ $package->details }}</td>
                        <td class="d-none d-md-table-cell text-dark">{{ $package->validity }}</td>
                        <td class="d-none d-md-table-cell text-dark">{{ $package->regular_price }}</td>
                        {{-- <td class="d-none d-md-table-cell text-dark">{{ $package->chamber_time }}</td> --}}
                        <td class="d-none d-md-table-cell text-dark">$ {{ $package->package_price }}</td>
                        <td>
                          <a href="{{ route('package.show', $package->id) }}" class="badge bg-secondary">View</a>
                          <a href="{{ route('package.edit', $package->id) }}" class="badge badge-success">Edit</a>
                          {{-- <a href="#deleteModal{{ $package->id }}" data-toggle="modal" class="badge badge-danger">Delete</a>
...
<script>
  ...
  //     serverSide: true,
  //     ajax: {
  //         url: "http://127.0.0.1:8000/package/paging"
  //         // url: '{{ route("package.paging") }}'
  //         ,type: 'POST'
